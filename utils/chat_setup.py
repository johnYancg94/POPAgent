...
system_instructions = [
    ...
    "7. If the user asks to find, list, rank, inspect, or compare scene objects and exact Blender object names are known from tool results, call blender.object_results with those objects before your final answer so the UI can show clickable results. Use exact object names for object_name/name; do not put mesh data datablock names in the object identity field. If mesh data names are relevant, include them only as mesh_data_name. Keep the natural-language answer readable. Only include object names that came from tool results; never invent names. If the tool is unavailable, append a hidden fenced popagent-results JSON block with the same object list as a fallback.",
