...
| `blender.nodes.inspect_materials` | 快照所有材质的节点树（节点/连线/图片） |
| `blender.nodes.validate_materials` | 对材质运行 PBR 诊断：Principled BSDF 连接、颜色空间、贴图连通性 |
| `blender.nodes.connect_pbr_textures` | 按名称匹配自动将 PBR 贴图连到 Principled BSDF 正确输入 |
| `blender.nodes.add_material_node` | 在材质节点树中添加节点 |
| `blender.nodes.connect_material_nodes` | 连接两个材质节点插口 |
| `blender.nodes.set_material_node_input` | 设置材质节点输入的默认值 |

#### 几何节点编辑（v1.2.1 新增）
| Skill | 功能 |
|---|---|
| `blender.nodes.inspect_geometry_nodes` | 快照所有 Geometry Nodes 修改器 |
| `blender.nodes.validate_geometry_nodes` | 诊断 GN：Group Input/Output 存在性、几何输出连线 |
| `blender.nodes.ensure_basic_geometry_nodes` | 脚手架：Group Input → Group Output 最小 GN 修改器 |
| `blender.nodes.add_geometry_node` | 在几何节点组中添加节点 |
| `blender.nodes.connect_geometry_nodes` | 连接两个几何节点插口 |
| `blender.nodes.set_geometry_node_input` | 设置几何节点输入的默认值 |
...
1. 选择 LLM 提供商（OpenAI / DeepSeek / Mimo / Anthropic）
...
| 区域 | 选项 |
|---|---|
| **API Keys** | OpenAI / DeepSeek / Mimo / Anthropic API Key 与 Base URL |
| **Read Aloud** | TTS 声线选择 |
| **Display** | 文字宽度微调 |
...
query_api() → _agent_query()
    │
    ├─ 选择 Provider (OpenAI / DeepSeek / Mimo / Anthropic)
