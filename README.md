...
| `blender.viewport_screenshot` | 截取 3D Viewport 截图；支持多模态的模型会作为图片阅读 | 只读，自动 |
...
> 视觉阅读当前视图需要先在 N 面板开启 Multimodal Images。开启后会显示图片输入区，支持 Ctrl+V 粘贴剪贴板图片/图片文件、选择当前 Blender 文件中的 Image datablock，或点击 + 从文件管理器选择图片。DeepSeek 当前暂时关闭多模态入口，后续模型升级后可在 provider 能力判断中开启。
